...
dist.knn <- function(A) {
    p    <- ncol(A)
    A2   <- tcrossprod(A)/p   # add a row number in the 1st column
...
pred <- function(i, y, d, x, subset=NULL, kmat, nlam, n, p, ctrlvar=NULL, shutdown.d=F) {    
...
compute <- function(range, y, d, x, subset=NULL, K, nlam, n, p, const=F, ctrlvar=NULL, shutdown.d=F) {
    if (const) {
      ...
      fit  <- sapply(range, function(i) pred(i=i, y=y, d=d, x=x, subset=subset,
                                             kmat=kmat, nlam=nlam, n=n, p=p, ctrlvar=ctrlvar, shutdown.d=shutdown.d))
    }
