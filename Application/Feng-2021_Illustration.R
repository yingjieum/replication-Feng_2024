...
#################################################
# This file replicates the results in Feng (2021)
...
source("Feng-2021_Illustration_funs.R")
...
range  <- 1:n
subset <- (d.full==0)
K  <- floor(n^(2*nlam/(2*nlam+1)))
pr <- mean(d.full)

# DPI selection of K for the full sample
K.dpi <- Kselect(K.rot=2*K, y=y.full[subset], x=x.full[subset,], nlam=nlam, odd.delta=2)

############################################
# Step 1: latent variables extraction

# Step 1.1: illustrate KNN matching
kmat      <- knn.index(A=x.full[,1:(p/2)], K=K.dpi)
...
PC    <- lpca(A=x.full[,-(1:(p/2))], index=index, nlam=5, n=n)  
...
for (j in 1:32) {
   ...
   result <- compute(range=range, y=y.t, d=d.full, x=x.full, subset=subset, 
                     K=K.dpi, nlam=nlam, n=n, p=p, const=F)
...
Kseq      <- floor(n^(2*nlam/(2*nlam+1))*c(.5, 1, 2))

# select K based on cross validation
K.dpi <- Kselect(K.rot=Kseq[3], y=y.full[subset], x=x.full[subset,], nlam=nlam, ctrlvar=NULL, odd.delta=2)
K.list <- c(Kseq, K.dpi)

reg.table <- matrix(NA, 12, 4)
...
for  (j in 1:4) {
   ...
   reg.table[2*j,2] <- paste("[", -round(se*1.96,3), ", ", round(se*1.96,3), "]", sep="")
}
reg.table[c(2,4,6,8),1] <- ""
...
K.dpi <- Kselect(K.rot=Kseq[3], y=y.sub[subset], x=x.sub[subset,], nlam=nlam, ctrlvar=NULL, odd.delta=2)
K.list <- c(Kseq, K.dpi)
...
for  (j in 1:4) {
   ...
   se     <- sqrt(mean(varphi^2))/sqrt(n)
   reg.table[2*j,3] <- paste("[", -round(se*1.96,3), ", ", round(se*1.96,3), "]", sep="")
}
...
for  (j in 1:4) {
   ...
   varphi <- (d.sub*(y.sub-theta1)/pr) - 
             ((d.sub*(result$yfit-theta0)+
              (1-d.sub)*(y.sub-result$yfit)*result$ps/(1-result$ps))/pr)
   se     <- sqrt(mean(varphi^2))/sqrt(n)
   reg.table[2*j,4] <- paste("[", -round(se*1.96,3), ", ", round(se*1.96,3), "]", sep="")
}

reg.table[9,]  <- c("", "0.005", "", "0.060")
reg.table[10,] <- c("","[-0.029, 0.014]","", "[-0.068, 0.036]")
reg.table[11,] <- c("", "-", "", "0.061")
reg.table[12,] <- c("", "-", "", "[-0.050, 0.061]")
...
K.dpi <- Kselect(K.rot=Kseq[3], y=y.full[subset], x=x.full[subset,], nlam=nlam, ctrlvar=ctrlvar[subset,], odd.delta=2)
K.list <- c(Kseq, K.dpi)

reg.table.ctr <- matrix(NA, 8, 4)
reg.table.ctr[c(1,3,5),1] <- Kseq; reg.table.ctr[c(2,4,6,8),1] <- ""
...
# influence fun.    
   varphi <- (d.full*(y.full-theta1)/pr) - 
             ((d.full*(result$yfit-theta0)+
              (1-d.full)*(y.full-result$yfit)*result$ps/(1-result$ps))/pr)
   se     <- sqrt(mean(varphi^2))/sqrt(n)
   reg.table.ctr[2*j,2] <- paste("[", -round(se*1.96,3), ", ", round(se*1.96,3), "]", sep="")
}
...
K.dpi <- Kselect(K.rot=Kseq[3], y=y.sub[subset], x=x.sub[subset,], nlam=nlam, ctrlvar=ctrl.sub[subset,], odd.delta=2)
K.list <- c(Kseq, K.dpi)
...
for  (j in 1:4) {
   ...
   varphi <- (d.sub*(y.sub-theta1)/pr) - 
             ((d.sub*(result$yfit-theta0)+
              (1-d.sub)*(y.sub-result$yfit)*result$ps/(1-result$ps))/pr)
   se     <- sqrt(mean(varphi^2))/sqrt(n)
   reg.table.ctr[2*j,3] <- paste("[", -round(se*1.96,3), ", ", round(se*1.96,3), "]", sep="")
}
...
for  (j in 1:4) {
   ...
   varphi <- (d.sub*(y.sub-theta1)/pr) - 
             ((d.sub*(result$yfit-theta0)+
              (1-d.sub)*(y.sub-result$yfit)*result$ps/(1-result$ps))/pr)
   se     <- sqrt(mean(varphi^2))/sqrt(n)
   reg.table.ctr[2*j,4] <- paste("[", -round(se*1.96,3), ", ", round(se*1.96,3), "]", sep="")
}
...
n.rgroup <- c(8, 2, 2)
rgroup   <- c("Local PCA, $K=$", "Acemoglu et al. (2016)", "Abadie and L'Hour (2021)")
rowname  <- c(Kseq[1], "", Kseq[2], "", Kseq[3], "", "$\\widehat{K}_{\\mathtt{DPI}}$", "", "Estimate", "CI for TE=0", "Estimate", "CI for TE=0")
...
n=length(y.full); subset <- (d.full==0); pr <- mean(d.full); 
range <- 1:n
K.dpi <- Kselect(K.rot=2*floor(n^(2/3)), y=y.full[subset], x=x.full[subset,], nlam=nlam, ctrlvar=NULL, odd.delta=2)
